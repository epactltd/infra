#!/bin/bash

# =========================================
# Docker Manager for Envelope SaaS
# Comprehensive Docker management tool
# Supports both Development and Production
# =========================================

set -euo pipefail

# Colors for output
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
CYAN='\033[0;36m'
NC='\033[0m' # No Color

# Script directory (docker/manager.sh)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# Go to infra root directory (one level up from docker/)
INFRA_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
cd "$INFRA_DIR"

# Docker compose files (relative to infra root)
DEV_COMPOSE="docker-compose.yml"
PROD_COMPOSE="docker-compose.prod.yml"

# Current environment (dev or prod)
ENV_MODE="dev"

# =========================================
# HELPER FUNCTIONS
# =========================================

print_header() {
    echo -e "${CYAN}========================================${NC}"
    echo -e "${CYAN}$1${NC}"
    echo -e "${CYAN}========================================${NC}"
}

print_success() {
    echo -e "${GREEN}✓ $1${NC}"
}

print_error() {
    echo -e "${RED}✗ $1${NC}"
}

print_warning() {
    echo -e "${YELLOW}⚠ $1${NC}"
}

print_info() {
    echo -e "${BLUE}ℹ $1${NC}"
}

get_compose_file() {
    if [ "$ENV_MODE" = "prod" ]; then
        echo "$PROD_COMPOSE"
    else
        echo "$DEV_COMPOSE"
    fi
}

get_compose_cmd() {
    local compose_file=$(get_compose_file)
    echo "docker compose -f $compose_file"
}

run_compose() {
    local cmd=$(get_compose_cmd)
    eval "$cmd $@"
}

require_sudo() {
    if ! sudo -n true 2>/dev/null; then
        print_warning "This operation requires sudo privileges"
        sudo -v
    fi
}

press_enter() {
    echo ""
    read -p "Press Enter to continue..."
}

select_service() {
    local prompt="${1:-Select service}"
    local default="${2:-all}"
    local result_var="${3:-SELECTED_SERVICE}"
    
    echo ""
    read -p "$prompt [api/tenant/hq/all] (press Enter for 'all'): " input
    
    # Trim whitespace and convert to lowercase
    input=$(echo "$input" | tr '[:upper:]' '[:lower:]' | xargs)
    
    # Validate input
    if [ -z "$input" ]; then
        # Empty input - use default
        eval "$result_var=\"$default\""
    elif [[ "$input" =~ ^(api|tenant|hq|all)$ ]]; then
        # Valid input
        eval "$result_var=\"$input\""
    else
        # Invalid input - use default and show warning
        print_warning "Invalid service '$input'. Using default: $default"
        eval "$result_var=\"$default\""
    fi
}

select_container() {
    local prompt="${1:-Select container}"
    local result_var="${2:-SELECTED_CONTAINER}"
    
    # Save original PS3 (handle unbound variable)
    local original_ps3="${PS3:-}"
    
    # Get list of running containers
    local containers=()
    while IFS= read -r line; do
        if [ -n "$line" ]; then
            containers+=("$line")
        fi
    done < <(docker ps --format "{{.Names}}" | grep -E "envelope|^" | head -20)
    
    if [ ${#containers[@]} -eq 0 ]; then
        print_error "No running containers found"
        return 1
    fi
    
    echo ""
    echo "$prompt:"
    echo ""
    echo -e "${CYAN}Running containers:${NC}"
    docker ps --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}" | grep -E "(NAMES|envelope)" || docker ps
    echo ""
    
    PS3="Select container [1-${#containers[@]}]: "
    
    local selected=""
    select container in "${containers[@]}"; do
        if [ -n "$container" ]; then
            selected="$container"
            break
        else
            print_error "Invalid selection. Please choose 1-${#containers[@]}"
        fi
    done
    
    # Restore PS3
    PS3="$original_ps3"
    
    # Set the result variable
    eval "$result_var=\"$selected\""
}

# =========================================
# ENVIRONMENT SELECTION
# =========================================

select_environment() {
    clear
    print_header "Select Environment"
    echo "1) Development (docker-compose.yml)"
    echo "2) Production (docker-compose.prod.yml)"
    echo ""
    read -p "Select [1-2]: " choice
    
    case $choice in
        1) ENV_MODE="dev" ;;
        2) ENV_MODE="prod" ;;
        *) 
            print_error "Invalid selection"
            select_environment
            return
            ;;
    esac
    
    print_success "Selected: $ENV_MODE environment"
    sleep 1
}

# =========================================
# DOCKER OPERATIONS
# =========================================

build_images() {
    print_header "Building Docker Images"
    local service
    select_service "Select service to build" "all" "service"
    
    if [ "$service" = "all" ]; then
        print_info "Building all images..."
        run_compose build
    else
        print_info "Building $service..."
        run_compose build "$service"
    fi
    
    print_success "Build completed"
    press_enter
}

rebuild_images() {
    print_header "Rebuilding Docker Images"
    local service
    select_service "Select service to rebuild" "all" "service"
    
    if [ "$service" = "all" ]; then
        print_info "Rebuilding all images (no cache)..."
        run_compose build --no-cache
    else
        print_info "Rebuilding $service (no cache)..."
        run_compose build --no-cache "$service"
    fi
    
    print_success "Rebuild completed"
    press_enter
}

start_services() {
    print_header "Starting Services"
    local service
    select_service "Select service to start" "all" "service"
    
    if [ "$service" = "all" ]; then
        print_info "Starting all services..."
        run_compose up -d
    else
        print_info "Starting $service..."
        run_compose up -d "$service"
    fi
    
    print_success "Services started"
    press_enter
}

stop_services() {
    print_header "Stopping Services"
    local service
    select_service "Select service to stop" "all" "service"
    
    if [ "$service" = "all" ]; then
        print_info "Stopping all services..."
        run_compose stop
    else
        print_info "Stopping $service..."
        run_compose stop "$service"
    fi
    
    print_success "Services stopped"
    press_enter
}

restart_services() {
    print_header "Restarting Services"
    local service
    select_service "Select service to restart" "all" "service"
    
    if [ "$service" = "all" ]; then
        print_info "Restarting all services..."
        run_compose restart
    else
        print_info "Restarting $service..."
        run_compose restart "$service"
    fi
    
    print_success "Services restarted"
    press_enter
}

down_services() {
    print_header "Stopping and Removing Services"
    print_warning "This will stop and remove all containers, networks, and volumes"
    read -p "Are you sure? [y/N]: " confirm
    
    if [[ "$confirm" =~ ^[Yy]$ ]]; then
        read -p "Remove volumes? [y/N]: " remove_volumes
        
        if [[ "$remove_volumes" =~ ^[Yy]$ ]]; then
            print_info "Stopping and removing services with volumes..."
            run_compose down -v
        else
            print_info "Stopping and removing services..."
            run_compose down
        fi
        
        print_success "Services stopped and removed"
    else
        print_info "Operation cancelled"
    fi
    
    press_enter
}

pull_images_with_retry() {
    print_header "Pull Images with Retry Logic"
    
    local MAX_RETRIES=3
    local RETRY_DELAY=10
    local PULL_TIMEOUT=300
    
    read -p "Max retries [3]: " retries_input
    MAX_RETRIES=${retries_input:-3}
    
    local images=("mariadb:10.11" "redis:7-alpine" "axllent/mailpit:latest")
    local failed_images=()
    local success_count=0
    
    print_info "Pulling external Docker images..."
    echo ""
    
    for image in "${images[@]}"; do
        local retry_count=0
        local success=false
        
        print_info "Pulling image: $image"
        
        while [ $retry_count -lt $MAX_RETRIES ] && [ "$success" = false ]; do
            if timeout $PULL_TIMEOUT docker pull "$image" 2>&1; then
                print_success "Successfully pulled $image"
                success=true
                success_count=$((success_count + 1))
            else
                retry_count=$((retry_count + 1))
                if [ $retry_count -lt $MAX_RETRIES ]; then
                    print_warning "Retrying in $RETRY_DELAY seconds... (Attempt $retry_count/$MAX_RETRIES)"
                    sleep $RETRY_DELAY
                else
                    print_error "Failed to pull $image after $MAX_RETRIES attempts"
                    failed_images+=("$image")
                fi
            fi
        done
        echo ""
    done
    
    print_info "Summary: $success_count/${#images[@]} images pulled successfully"
    
    if [ ${#failed_images[@]} -gt 0 ]; then
        print_warning "Failed images (will use cached if available):"
        for img in "${failed_images[@]}"; do
            echo "  - $img"
        done
    fi
    
    press_enter
}

# =========================================
# IMAGE MANAGEMENT
# =========================================

list_images() {
    print_header "Docker Images"
    echo ""
    docker images | grep -E "(REPOSITORY|envelope)" || docker images
    echo ""
    press_enter
}

delete_image() {
    print_header "Delete Docker Image"
    echo ""
    docker images | grep envelope || docker images
    echo ""
    read -p "Image name or ID: " image_name
    
    if [ -z "$image_name" ]; then
        print_error "Image name required"
        press_enter
        return
    fi
    
    print_warning "Deleting image: $image_name"
    read -p "Are you sure? [y/N]: " confirm
    
    if [[ "$confirm" =~ ^[Yy]$ ]]; then
        docker rmi "$image_name" || print_error "Failed to delete image"
        print_success "Image deleted"
    else
        print_info "Operation cancelled"
    fi
    
    press_enter
}

delete_all_images() {
    print_header "Delete All Envelope Images"
    print_warning "This will delete ALL envelope-related images"
    read -p "Are you sure? [y/N]: " confirm
    
    if [[ "$confirm" =~ ^[Yy]$ ]]; then
        print_info "Deleting envelope images..."
        docker images | grep envelope | awk '{print $3}' | xargs -r docker rmi -f || true
        print_success "All envelope images deleted"
    else
        print_info "Operation cancelled"
    fi
    
    press_enter
}

# =========================================
# CONTAINER MANAGEMENT
# =========================================

list_containers() {
    print_header "Docker Containers"
    echo ""
    echo -e "${CYAN}Running containers:${NC}"
    docker ps --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}" | grep -E "(NAMES|envelope)" || docker ps
    echo ""
    echo -e "${CYAN}All containers:${NC}"
    docker ps -a --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}" | grep -E "(NAMES|envelope)" || docker ps -a
    echo ""
    press_enter
}

show_logs() {
    print_header "Container Logs"
    local service
    select_service "Select service to view logs" "all" "service"
    
    if [ "$service" = "all" ]; then
        print_info "Showing logs for all services (last 50 lines)..."
        run_compose logs --tail=50 -f
    else
        read -p "Number of lines [50]: " lines
        lines=${lines:-50}
        print_info "Showing logs for $service (last $lines lines)..."
        run_compose logs --tail="$lines" -f "$service"
    fi
}

# =========================================
# DEBUGGING & DIAGNOSTICS
# =========================================

show_status() {
    print_header "Service Status"
    echo ""
    run_compose ps
    echo ""
    press_enter
}

show_health() {
    print_header "Container Health Status"
    echo ""
    docker ps --format "table {{.Names}}\t{{.Status}}" | grep envelope || docker ps
    echo ""
    press_enter
}

inspect_container() {
    print_header "Inspect Container"
    local container_name
    select_container "Select container to inspect" "container_name"
    
    if [ -z "$container_name" ]; then
        print_error "No container selected"
        press_enter
        return
    fi
    
    echo ""
    print_info "Inspecting container: $container_name"
    docker inspect "$container_name" | less
}

exec_into_container() {
    print_header "Execute into Container"
    local container_name
    select_container "Select container to enter" "container_name"
    
    if [ -z "$container_name" ]; then
        print_error "No container selected"
        press_enter
        return
    fi
    
    print_info "Entering container: $container_name"
    print_info "Type 'exit' to leave"
    docker exec -it "$container_name" /bin/bash || docker exec -it "$container_name" /bin/sh
}

diagnose_service() {
    print_header "Service Diagnostics"
    require_sudo
    
    local service_name
    select_service "Select service to diagnose" "all" "service_name"
    
    if [ "$service_name" = "all" ]; then
        services=("api" "hq" "tenant")
    else
        services=("$service_name")
    fi
    
    echo ""
    echo "1. Checking Docker Compose Services Status..."
    echo "--------------------------------------------"
    run_compose ps
    echo ""
    
    for service in "${services[@]}"; do
        print_header "Diagnosing $service Service"
        
        # Get container name based on service
        case $service in
            api) container="envelope_api" ;;
            hq) container="envelope_hq" ;;
            tenant) container="envelope_tenant" ;;
            *) 
                print_error "Unknown service: $service"
                continue
                ;;
        esac
        
        echo "2. Checking ${service^} Container Logs (last 100 lines)..."
        echo "--------------------------------------------"
        run_compose logs "$service" --tail=100
        echo ""
        
        echo "3. Checking if ${service^} Container is Running..."
        echo "--------------------------------------------"
        if docker ps | grep -q "$container"; then
            print_success "${service^} container is running"
            echo ""
            echo "4. Checking Container Health..."
            echo "--------------------------------------------"
            docker inspect "$container" --format='{{.State.Status}} - Health: {{.State.Health.Status}}' 2>/dev/null || echo "Health check not available"
            echo ""
            echo "5. Checking Port Mappings..."
            echo "--------------------------------------------"
            docker port "$container" 2>/dev/null || echo "Cannot get port mappings"
            echo ""
            
            # Service-specific diagnostics
            if [ "$service" = "api" ]; then
                echo "6. Testing API Endpoint..."
                echo "--------------------------------------------"
                curl -s -o /dev/null -w "HTTP Status: %{http_code}\n" http://localhost:8008 || echo "Cannot connect to API"
                echo ""
                echo "7. Checking Octane Process Inside Container..."
                echo "--------------------------------------------"
                docker exec "$container" ps aux | grep -E "(octane|php|supervisor)" 2>/dev/null || echo "Cannot check processes"
                echo ""
                echo "8. Checking Supervisor Status..."
                echo "--------------------------------------------"
                docker exec "$container" supervisorctl status 2>&1 || echo "Cannot check supervisor"
                echo ""
                echo "9. Checking Octane Logs..."
                echo "--------------------------------------------"
                docker exec "$container" tail -50 /var/www/html/storage/logs/octane.log 2>/dev/null || echo "Cannot read Octane logs"
                echo ""
                echo "10. Checking Laravel Logs..."
                echo "--------------------------------------------"
                docker exec "$container" tail -50 /var/www/html/storage/logs/laravel.log 2>/dev/null || echo "Cannot read Laravel logs"
                echo ""
                echo "11. Checking Environment Variables..."
                echo "--------------------------------------------"
                docker exec "$container" env | grep -E "(OCTANE|REVERB|DB_|APP_)" || echo "Cannot check environment"
                echo ""
                echo "12. Checking Database Connection..."
                echo "--------------------------------------------"
                docker exec "$container" php /var/www/html/artisan tinker --execute="try { DB::connection()->getPdo(); echo 'DB Connected'; } catch (Exception \$e) { echo 'DB Error: ' . \$e->getMessage(); }" 2>&1 || echo "Cannot test DB connection"
            elif [ "$service" = "hq" ] || [ "$service" = "tenant" ]; then
                local port="3000"
                if [ "$service" = "tenant" ]; then
                    port="3001"
                fi
                echo "6. Testing ${service^} Frontend Endpoint..."
                echo "--------------------------------------------"
                curl -s -o /dev/null -w "HTTP Status: %{http_code}\n" "http://localhost:$port" || echo "Cannot connect to ${service^}"
                echo ""
                echo "7. Checking Node Processes..."
                echo "--------------------------------------------"
                docker exec "$container" ps aux | grep -E "(node|nuxt|pnpm)" 2>/dev/null || echo "Cannot check processes"
                echo ""
                echo "8. Checking Environment Variables..."
                echo "--------------------------------------------"
                docker exec "$container" env | grep -E "(NODE_ENV|NUXT|PORT|BASE_URL)" || echo "Cannot check environment"
            fi
        else
            print_error "${service^} container is NOT running"
            echo ""
            echo "4. Checking why container exited..."
            echo "--------------------------------------------"
            run_compose logs "$service" --tail=50
            echo ""
            echo "5. Checking Container Exit Code..."
            echo "--------------------------------------------"
            docker inspect "$container" --format='Exit Code: {{.State.ExitCode}}' 2>/dev/null || echo "Container not found"
        fi
        
        echo ""
    done
    
    print_header "Diagnostic Complete"
    press_enter
}

test_services() {
    print_header "Testing Services"
    
    test_service() {
        local name=$1
        local url=$2
        local expected_status=${3:-200}
        
        echo -n "Testing $name... "
        
        local http_code=$(curl -s -o /dev/null -w "%{http_code}" "$url" 2>/dev/null || echo "000")
        
        if echo "$http_code" | grep -q "$expected_status"; then
            print_success "OK (HTTP $http_code)"
            return 0
        else
            print_error "FAILED (HTTP $http_code, expected $expected_status)"
            return 1
        fi
    }
    
    echo "Checking Docker containers..."
    if ! run_compose ps | grep -q "Up"; then
        print_warning "Some containers may not be running"
    fi
    
    echo ""
    echo "Testing services..."
    echo ""
    
    # Test API
    echo "API Service:"
    echo "   URL: http://localhost:8008"
    test_service "API Root" "http://localhost:8008" "200"
    test_service "API Health" "http://localhost:8008/up" "200"
    echo ""
    
    # Test HQ Frontend
    echo "HQ Frontend:"
    echo "   URL: http://localhost:3000"
    test_service "HQ Frontend" "http://localhost:3000" "200"
    echo ""
    
    # Test Tenant Frontend
    echo "Tenant Frontend:"
    echo "   URL: http://localhost:3001"
    test_service "Tenant Frontend" "http://localhost:3001/auth/login" "200"
    echo ""
    
    # Test Mailpit
    echo "Mailpit:"
    echo "   URL: http://localhost:8025"
    test_service "Mailpit UI" "http://localhost:8025" "200"
    echo ""
    
    print_header "Health Check Complete"
    press_enter
}

fix_service_cache() {
    print_header "Fix Service - Clear Caches"
    require_sudo
    
    local service_name
    select_service "Select service to fix cache" "all" "service_name"
    
    if [ "$service_name" = "all" ]; then
        services=("api" "hq" "tenant")
    else
        services=("$service_name")
    fi
    
    for service in "${services[@]}"; do
        print_header "Fixing $service Service"
        
        # Get container name based on service
        case $service in
            api) container="envelope_api" ;;
            hq) container="envelope_hq" ;;
            tenant) container="envelope_tenant" ;;
            *) 
                print_error "Unknown service: $service"
                continue
                ;;
        esac
        
        echo "1. Stopping ${service^} container..."
        run_compose stop "$service"
        
        echo ""
        echo "2. Clearing cache files..."
        if docker ps -a | grep -q "$container"; then
            if [ "$service" = "api" ]; then
                # Laravel cache clearing
                docker exec "$container" rm -f /var/www/html/bootstrap/cache/packages.php 2>/dev/null || echo "   (packages.php not found)"
                docker exec "$container" rm -f /var/www/html/bootstrap/cache/services.php 2>/dev/null || echo "   (services.php not found)"
            elif [ "$service" = "hq" ] || [ "$service" = "tenant" ]; then
                # Nuxt cache clearing
                docker exec "$container" rm -rf /app/.nuxt 2>/dev/null || echo "   (.nuxt not found)"
                docker exec "$container" rm -rf /app/.output 2>/dev/null || echo "   (.output not found)"
            fi
        fi
        
        echo ""
        echo "3. Starting ${service^} container..."
        run_compose up -d "$service"
        
        echo ""
        echo "4. Waiting for container to start..."
        sleep 10
        
        echo ""
        echo "5. Clearing runtime caches inside container..."
        if [ "$service" = "api" ]; then
            # Laravel cache commands
            docker exec "$container" php artisan config:clear 2>/dev/null || echo "   (config:clear failed)"
            docker exec "$container" php artisan cache:clear 2>/dev/null || echo "   (cache:clear failed)"
            docker exec "$container" php artisan optimize:clear 2>/dev/null || echo "   (optimize:clear failed)"
            echo ""
            echo "6. Regenerating composer autoload..."
            docker exec "$container" composer dump-autoload 2>/dev/null || echo "   (composer dump-autoload failed)"
            echo ""
            echo "7. Waiting for Octane to start..."
            sleep 5
        elif [ "$service" = "hq" ] || [ "$service" = "tenant" ]; then
            # Nuxt doesn't need runtime cache clearing, it's handled on startup
            echo "   (Nuxt caches cleared, service will rebuild on next request)"
            sleep 3
        fi
        
        echo ""
        echo "8. Checking container status..."
        run_compose ps "$service"
        
        echo ""
        echo "9. Checking logs..."
        run_compose logs "$service" --tail=30
        
        echo ""
    done
    
    print_header "Cache clearing complete!"
    press_enter
}

configure_docker() {
    print_header "Configure Docker Daemon"
    require_sudo
    
    local DAEMON_JSON="/etc/docker/daemon.json"
    local DAEMON_JSON_BACKUP="/etc/docker/daemon.json.backup.$(date +%Y%m%d_%H%M%S)"
    
    print_info "Configuring Docker daemon for better network reliability..."
    
    # Create daemon.json if it doesn't exist
    if [ ! -f "$DAEMON_JSON" ]; then
        print_info "Creating new daemon.json..."
        mkdir -p /etc/docker
        echo '{}' > "$DAEMON_JSON"
    else
        print_info "Backing up existing daemon.json..."
        cp "$DAEMON_JSON" "$DAEMON_JSON_BACKUP"
        print_success "Backup saved to: $DAEMON_JSON_BACKUP"
    fi
    
    # Check if jq is available
    if command -v jq &> /dev/null; then
        local config=$(cat "$DAEMON_JSON" 2>/dev/null || echo '{}')
        config=$(echo "$config" | jq '
            .max-concurrent-downloads = 3 |
            .max-concurrent-uploads = 5 |
            .default-ulimits.nofile.soft = 64000 |
            .default-ulimits.nofile.hard = 64000 |
            .log-driver = "json-file" |
            .log-opts."max-size" = "10m" |
            .log-opts."max-file" = "3"
        ')
        echo "$config" > "$DAEMON_JSON"
        print_success "Docker daemon configuration updated"
    else
        print_warning "jq not found. Manual configuration recommended."
        print_info "Add these settings to $DAEMON_JSON:"
        cat << EOF
{
  "max-concurrent-downloads": 3,
  "max-concurrent-uploads": 5,
  "default-ulimits": {
    "nofile": {
      "soft": 64000,
      "hard": 64000
    }
  },
  "log-driver": "json-file",
  "log-opts": {
    "max-size": "10m",
    "max-file": "3"
  }
}
EOF
        press_enter
        return
    fi
    
    # Restart Docker daemon
    print_info "Restarting Docker daemon..."
    if systemctl restart docker 2>/dev/null || service docker restart 2>/dev/null; then
        print_success "Docker daemon restarted"
    else
        print_warning "Could not restart Docker daemon automatically"
        print_info "Please restart Docker manually: sudo systemctl restart docker"
    fi
    
    press_enter
}

cleanup_docker() {
    print_header "Docker Cleanup"
    echo ""
    echo "This will remove:"
    echo "  - Stopped containers"
    echo "  - Unused networks"
    echo "  - Unused images"
    echo "  - Build cache"
    echo ""
    read -p "Continue? [y/N]: " confirm
    
    if [[ "$confirm" =~ ^[Yy]$ ]]; then
        print_info "Cleaning up Docker..."
        docker system prune -af --volumes
        print_success "Cleanup completed"
    else
        print_info "Operation cancelled"
    fi
    
    press_enter
}

# =========================================
# TENANT & QUEUE MANAGEMENT
# =========================================

check_queue_status() {
    print_header "Queue Status"
    local container="envelope_worker"
    
    if ! docker ps --format "{{.Names}}" | grep -q "^${container}$"; then
        print_error "Worker container '$container' is not running!"
        print_info "Start it with: docker compose up -d worker"
        press_enter
        return
    fi
    
    echo "1. Checking Queue Connection..."
    echo "--------------------------------------------"
    docker exec "$container" php /var/www/html/artisan queue:monitor 2>&1 || echo "Queue monitor not available"
    echo ""
    
    echo "2. Checking Pending Jobs..."
    echo "--------------------------------------------"
    docker exec "$container" php /var/www/html/artisan queue:work --once --verbose 2>&1 | head -20 || echo "Cannot check jobs"
    echo ""
    
    echo "3. Checking Failed Jobs..."
    echo "--------------------------------------------"
    local failed_count=$(docker exec "$container" php /var/www/html/artisan queue:failed 2>&1 | grep -c "ID" || echo "0")
    if [ "$failed_count" -gt 1 ]; then
        print_warning "Found $((failed_count - 1)) failed jobs"
        docker exec "$container" php /var/www/html/artisan queue:failed
    else
        print_success "No failed jobs"
    fi
    echo ""
    
    echo "4. Checking Queue Tables..."
    echo "--------------------------------------------"
    docker exec "$container" php /var/www/html/artisan tinker --execute="
        try {
            \$pending = DB::table('jobs')->count();
            \$failed = DB::table('failed_jobs')->count();
            echo 'Pending jobs: ' . \$pending . PHP_EOL;
            echo 'Failed jobs: ' . \$failed . PHP_EOL;
        } catch (Exception \$e) {
            echo 'Error: ' . \$e->getMessage() . PHP_EOL;
        }
    " 2>&1
    echo ""
    
    press_enter
}

check_tenant_seeding() {
    print_header "Tenant Seeding Status"
    local container="envelope_api"
    
    if ! docker ps --format "{{.Names}}" | grep -q "^${container}$"; then
        print_error "API container '$container' is not running!"
        print_info "To start the API container, use option 4 (Start Services)"
        press_enter
        return
    fi
    
    echo "1. Listing All Tenants..."
    echo "--------------------------------------------"
    docker exec "$container" php /var/www/html/artisan tenants:list 2>&1
    echo ""
    
    read -p "Enter tenant ID to check (or press Enter for all): " tenant_id
    
    if [ -z "$tenant_id" ]; then
        echo "2. Checking All Tenant Databases..."
        echo "--------------------------------------------"
        docker exec "$container" php /var/www/html/artisan tinker --execute="
            \$tenants = \App\Models\Central\Tenant::all();
            foreach (\$tenants as \$tenant) {
                echo 'Tenant ID: ' . \$tenant->id . ' - ' . \$tenant->id . PHP_EOL;
                try {
                    \$tenant->run(function () {
                        \$users = \App\Models\Tenant\User::count();
                        \$perms = \App\Models\Tenant\Permission::count();
                        \$drops = \App\Models\Tenant\Dropdown::count();
                        echo '  Users: ' . \$users . ', Permissions: ' . \$perms . ', Dropdowns: ' . \$drops . PHP_EOL;
                    });
                } catch (Exception \$e) {
                    echo '  ERROR: ' . \$e->getMessage() . PHP_EOL;
                }
            }
        " 2>&1
    else
        echo "2. Checking Tenant $tenant_id Database..."
        echo "--------------------------------------------"
        docker exec "$container" php /var/www/html/artisan tinker --execute="
            \$tenant = \App\Models\Central\Tenant::find($tenant_id);
            if (!\$tenant) {
                echo 'Tenant not found!' . PHP_EOL;
                exit;
            }
            echo 'Tenant ID: ' . \$tenant->id . PHP_EOL;
            try {
                \$tenant->run(function () {
                    echo 'Users: ' . \App\Models\Tenant\User::count() . PHP_EOL;
                    echo 'Permissions: ' . \App\Models\Tenant\Permission::count() . PHP_EOL;
                    echo 'Dropdowns: ' . \App\Models\Tenant\Dropdown::count() . PHP_EOL;
                    echo 'Tags: ' . \App\Models\Tenant\Tag::count() . PHP_EOL;
                    echo 'Settings: ' . \App\Models\Tenant\Setting::count() . PHP_EOL;
                    echo 'Placeholders: ' . \App\Models\Tenant\Placeholder::count() . PHP_EOL;
                });
            } catch (Exception \$e) {
                echo 'ERROR: ' . \$e->getMessage() . PHP_EOL;
            }
        " 2>&1
    fi
    echo ""
    
    echo "3. Checking Queue Jobs for Tenant Setup..."
    echo "--------------------------------------------"
    docker exec "$container" php /var/www/html/artisan tinker --execute="
        try {
            \$jobs = DB::table('jobs')
                ->where('queue', 'default')
                ->whereRaw('payload LIKE ?', ['%SetupTenantDatabase%'])
                ->orWhereRaw('payload LIKE ?', ['%MigrateDatabase%'])
                ->orWhereRaw('payload LIKE ?', ['%SeedDatabase%'])
                ->get();
            echo 'Found ' . \$jobs->count() . ' tenant setup jobs in queue' . PHP_EOL;
            foreach (\$jobs as \$job) {
                echo '  Job ID: ' . \$job->id . ' - Attempts: ' . \$job->attempts . PHP_EOL;
            }
        } catch (Exception \$e) {
            echo 'Error: ' . \$e->getMessage() . PHP_EOL;
        }
    " 2>&1
    echo ""
    
    press_enter
}

retry_failed_jobs() {
    print_header "Retry Failed Jobs"
    local container="envelope_worker"
    
    if ! docker ps --format "{{.Names}}" | grep -q "^${container}$"; then
        print_error "Worker container '$container' is not running!"
        print_info "Start it with: docker compose up -d worker"
        press_enter
        return
    fi
    
    echo "Failed jobs:"
    docker exec "$container" php /var/www/html/artisan queue:failed
    echo ""
    
    read -p "Retry all failed jobs? [y/N]: " confirm
    if [[ "$confirm" =~ ^[Yy]$ ]]; then
        print_info "Retrying all failed jobs..."
        docker exec "$container" php /var/www/html/artisan queue:retry all
        print_success "Failed jobs retried"
    else
        read -p "Enter job ID to retry (or press Enter to cancel): " job_id
        if [ -n "$job_id" ]; then
            docker exec "$container" php /var/www/html/artisan queue:retry "$job_id"
            print_success "Job $job_id retried"
        fi
    fi
    
    press_enter
}

manually_seed_tenant() {
    print_header "Manually Seed Tenant"
    local container="envelope_api"
    
    # Check if container exists and is running
    if ! docker ps --format "{{.Names}}" | grep -q "^${container}$"; then
        print_error "API container '$container' is not running!"
        echo ""
        print_info "Available containers:"
        docker ps --format "table {{.Names}}\t{{.Status}}" | grep -E "(NAMES|envelope)" || docker ps
        echo ""
        print_info "To start the API container, use option 4 (Start Services)"
        press_enter
        return
    fi
    
    echo "Available tenants:"
    docker exec "$container" php /var/www/html/artisan tenants:list 2>&1
    echo ""
    
    read -p "Enter tenant ID to seed: " tenant_id
    if [ -z "$tenant_id" ]; then
        print_error "Tenant ID required"
        press_enter
        return
    fi
    
    echo ""
    echo "Seeding options:"
    echo "1) Run all seeders (TenantDatabaseSeeder)"
    echo "2) Run specific seeder"
    echo "3) Run migrations and seeders"
    read -p "Select option [1-3]: " option
    
    case $option in
        1)
            print_info "Running all tenant seeders..."
            docker exec "$container" php /var/www/html/artisan tenants:seed --tenants="$tenant_id" --class=Database\\Seeders\\TenantDatabaseSeeder
            ;;
        2)
            echo "Available seeders:"
            echo "  - UserSeeder"
            echo "  - PermissionSeeder"
            echo "  - DropdownSeeder"
            echo "  - TagSeeder"
            echo "  - SettingSeeder"
            echo "  - PlaceholderSeeder"
            read -p "Enter seeder class name: " seeder
            docker exec "$container" php /var/www/html/artisan tenants:seed --tenants="$tenant_id" --class="Database\\Seeders\\Tenant\\$seeder"
            ;;
        3)
            print_info "Running migrations..."
            docker exec "$container" php /var/www/html/artisan tenants:migrate --tenants="$tenant_id"
            print_info "Running seeders..."
            docker exec "$container" php /var/www/html/artisan tenants:seed --tenants="$tenant_id" --class=Database\\Seeders\\TenantDatabaseSeeder
            ;;
        *)
            print_error "Invalid option"
            press_enter
            return
            ;;
    esac
    
    print_success "Seeding completed"
    press_enter
}

migrate_tenant() {
    print_header "Migrate Tenant Database"
    local container="envelope_api"
    
    if ! docker ps --format "{{.Names}}" | grep -q "^${container}$"; then
        print_error "API container '$container' is not running!"
        print_info "To start the API container, use option 4 (Start Services)"
        press_enter
        return
    fi
    
    echo "Available tenants:"
    docker exec "$container" php /var/www/html/artisan tenants:list 2>&1
    echo ""
    
    read -p "Enter tenant ID to migrate (or 'all' for all tenants): " tenant_id
    if [ -z "$tenant_id" ]; then
        print_error "Tenant ID required"
        press_enter
        return
    fi
    
    if [ "$tenant_id" = "all" ]; then
        print_info "Running migrations for all tenants using tenants:migrate..."
        docker exec "$container" php /var/www/html/artisan tenants:migrate
    else
        print_info "Running migrations for tenant $tenant_id using tenants:migrate..."
        docker exec "$container" php /var/www/html/artisan tenants:migrate --tenants="$tenant_id"
    fi
    
    print_success "Migration completed"
    press_enter
}

fresh_migrate_tenant() {
    print_header "Fresh Migrate Tenant Database"
    local container="envelope_api"
    
    if ! docker ps --format "{{.Names}}" | grep -q "^${container}$"; then
        print_error "API container '$container' is not running!"
        print_info "To start the API container, use option 4 (Start Services)"
        press_enter
        return
    fi
    
    echo "Available tenants:"
    docker exec "$container" php /var/www/html/artisan tenants:list 2>&1
    echo ""
    
    read -p "Enter tenant ID to fresh migrate (or 'all' for all tenants): " tenant_id
    if [ -z "$tenant_id" ]; then
        print_error "Tenant ID required"
        press_enter
        return
    fi
    
    print_warning "WARNING: This will DROP ALL TABLES and re-run migrations!"
    print_warning "All data in the tenant database(s) will be LOST!"
    read -p "Are you sure you want to continue? [y/N]: " confirm
    
    if [[ ! "$confirm" =~ ^[Yy]$ ]]; then
        print_info "Operation cancelled"
        press_enter
        return
    fi
    
    if [ "$tenant_id" = "all" ]; then
        print_info "Running fresh migrations for all tenants using tenants:migrate-fresh..."
        docker exec "$container" php /var/www/html/artisan tenants:migrate-fresh
    else
        print_info "Running fresh migrations for tenant $tenant_id using tenants:migrate-fresh..."
        docker exec "$container" php /var/www/html/artisan tenants:migrate-fresh --tenants="$tenant_id"
    fi
    
    print_success "Fresh migration completed"
    press_enter
}

fresh_migrate_and_seed_tenant() {
    print_header "Fresh Migrate and Seed Tenant Database"
    local container="envelope_api"
    
    if ! docker ps --format "{{.Names}}" | grep -q "^${container}$"; then
        print_error "API container '$container' is not running!"
        print_info "To start the API container, use option 4 (Start Services)"
        press_enter
        return
    fi
    
    echo "Available tenants:"
    docker exec "$container" php /var/www/html/artisan tenants:list 2>&1
    echo ""
    
    read -p "Enter tenant ID to fresh migrate and seed (or 'all' for all tenants): " tenant_id
    if [ -z "$tenant_id" ]; then
        print_error "Tenant ID required"
        press_enter
        return
    fi
    
    print_warning "WARNING: This will DROP ALL TABLES, re-run migrations, and seed the database!"
    print_warning "All data in the tenant database(s) will be LOST!"
    read -p "Are you sure you want to continue? [y/N]: " confirm
    
    if [[ ! "$confirm" =~ ^[Yy]$ ]]; then
        print_info "Operation cancelled"
        press_enter
        return
    fi
    
    if [ "$tenant_id" = "all" ]; then
        print_info "Running fresh migrations for all tenants using tenants:migrate-fresh..."
        docker exec "$container" php /var/www/html/artisan tenants:migrate-fresh
        print_info "Seeding all tenants using tenants:seed..."
        docker exec "$container" php /var/www/html/artisan tenants:seed --class=Database\\Seeders\\TenantDatabaseSeeder
    else
        print_info "Running fresh migrations for tenant $tenant_id using tenants:migrate-fresh..."
        docker exec "$container" php /var/www/html/artisan tenants:migrate-fresh --tenants="$tenant_id"
        print_info "Seeding tenant $tenant_id using tenants:seed..."
        docker exec "$container" php /var/www/html/artisan tenants:seed --tenants="$tenant_id" --class=Database\\Seeders\\TenantDatabaseSeeder
    fi
    
    print_success "Fresh migration and seeding completed"
    press_enter
}

fresh_migrate_central() {
    print_header "Fresh Migrate Central Database"
    local container="envelope_api"
    
    if ! docker ps --format "{{.Names}}" | grep -q "^${container}$"; then
        print_error "API container '$container' is not running!"
        print_info "To start the API container, use option 4 (Start Services)"
        press_enter
        return
    fi
    
    print_warning "WARNING: This will DROP ALL TABLES in the central database and re-run migrations!"
    print_warning "All central database data will be LOST!"
    read -p "Are you sure you want to continue? [y/N]: " confirm
    
    if [[ ! "$confirm" =~ ^[Yy]$ ]]; then
        print_info "Operation cancelled"
        press_enter
        return
    fi
    
    print_info "Running fresh migrations for central database..."
    docker exec "$container" php /var/www/html/artisan migrate:fresh --force
    
    print_success "Fresh migration completed"
    press_enter
}

fresh_migrate_and_seed_central() {
    print_header "Fresh Migrate and Seed Central Database"
    local container="envelope_api"
    
    if ! docker ps --format "{{.Names}}" | grep -q "^${container}$"; then
        print_error "API container '$container' is not running!"
        print_info "To start the API container, use option 4 (Start Services)"
        press_enter
        return
    fi
    
    print_warning "WARNING: This will DROP ALL TABLES, re-run migrations, and seed the central database!"
    print_warning "All central database data will be LOST!"
    read -p "Are you sure you want to continue? [y/N]: " confirm
    
    if [[ ! "$confirm" =~ ^[Yy]$ ]]; then
        print_info "Operation cancelled"
        press_enter
        return
    fi
    
    print_info "Running fresh migrations for central database..."
    docker exec "$container" php /var/www/html/artisan migrate:fresh --seed --force
    
    print_success "Fresh migration and seeding completed"
    press_enter
}

run_artisan_command() {
    print_header "Run Artisan Command"
    local container="envelope_api"
    
    if ! docker ps --format "{{.Names}}" | grep -q "^${container}$"; then
        print_error "API container '$container' is not running!"
        print_info "To start the API container, use option 4 (Start Services)"
        press_enter
        return
    fi
    
    # Get available commands and categorize them
    echo "Fetching available artisan commands..."
    local all_commands=$(docker exec "$container" php /var/www/html/artisan list --format=json 2>/dev/null | jq -r '.commands[] | "\(.name)|\(.description)"' 2>/dev/null || docker exec "$container" php /var/www/html/artisan list 2>&1)
    
    # Common tenant commands
    local tenant_commands=(
        "tenants:list|List all tenants"
        "tenants:migrate|Run migrations for tenant(s)"
        "tenants:migrate-fresh|Drop all tables and re-run migrations for tenant(s)"
        "tenants:seed|Seed tenant database(s)"
        "tenants:rollback|Rollback migrations for tenant(s)"
        "tenants:run|Run a command for tenant(s)"
        "tenants:backup|Backup tenant databases"
        "tenants:restore|Restore a tenant database from backup"
    )
    
    # Common queue commands
    local queue_commands=(
        "queue:work|Process jobs from the queue"
        "queue:listen|Listen to a given queue"
        "queue:restart|Restart queue worker daemons"
        "queue:failed|List all of the failed queue jobs"
        "queue:retry|Retry a failed queue job"
        "queue:flush|Flush all of the failed queue jobs"
        "queue:forget|Delete a failed queue job"
    )
    
    # Common database commands
    local db_commands=(
        "migrate|Run the database migrations"
        "migrate:fresh|Drop all tables and re-run all migrations"
        "migrate:refresh|Reset and re-run all migrations"
        "migrate:rollback|Rollback the last database migration"
        "db:seed|Seed the database with records"
        "db:show|Display information about the given database"
        "db:table|Display information about the given database table"
    )
    
    # Common cache/optimize commands
    local cache_commands=(
        "cache:clear|Flush the application cache"
        "config:clear|Remove the configuration cache file"
        "config:cache|Create a cache file for faster configuration loading"
        "route:clear|Remove the route cache file"
        "route:cache|Create a route cache file for faster route registration"
        "view:clear|Clear all compiled view files"
        "optimize:clear|Remove the cached bootstrap files"
        "optimize|Cache the framework bootstrap files"
    )
    
    echo ""
    echo "Select a command category or type a custom command:"
    echo ""
    echo "  [1] Tenant Commands"
    echo "  [2] Queue Commands"
    echo "  [3] Database Commands"
    echo "  [4] Cache/Optimize Commands"
    echo "  [5] Type Custom Command"
    echo "  [0] Cancel"
    echo ""
    read -p "Select option [0-5]: " category_choice
    
    local selected_command=""
    local command_args=""
    
    case $category_choice in
        1)
            echo ""
            echo "Tenant Commands:"
            for i in "${!tenant_commands[@]}"; do
                local num=$((i + 1))
                local cmd_info="${tenant_commands[$i]}"
                local cmd_name="${cmd_info%%|*}"
                local cmd_desc="${cmd_info#*|}"
                echo "  $num) $cmd_name - $cmd_desc"
            done
            echo ""
            read -p "Select command [1-${#tenant_commands[@]}]: " cmd_choice
            if [ -n "$cmd_choice" ] && [ "$cmd_choice" -ge 1 ] && [ "$cmd_choice" -le "${#tenant_commands[@]}" ]; then
                selected_command="${tenant_commands[$((cmd_choice - 1))]%%|*}"
            fi
            ;;
        2)
            echo ""
            echo "Queue Commands:"
            for i in "${!queue_commands[@]}"; do
                local num=$((i + 1))
                local cmd_info="${queue_commands[$i]}"
                local cmd_name="${cmd_info%%|*}"
                local cmd_desc="${cmd_info#*|}"
                echo "  $num) $cmd_name - $cmd_desc"
            done
            echo ""
            read -p "Select command [1-${#queue_commands[@]}]: " cmd_choice
            if [ -n "$cmd_choice" ] && [ "$cmd_choice" -ge 1 ] && [ "$cmd_choice" -le "${#queue_commands[@]}" ]; then
                selected_command="${queue_commands[$((cmd_choice - 1))]%%|*}"
            fi
            ;;
        3)
            echo ""
            echo "Database Commands:"
            for i in "${!db_commands[@]}"; do
                local num=$((i + 1))
                local cmd_info="${db_commands[$i]}"
                local cmd_name="${cmd_info%%|*}"
                local cmd_desc="${cmd_info#*|}"
                echo "  $num) $cmd_name - $cmd_desc"
            done
            echo ""
            read -p "Select command [1-${#db_commands[@]}]: " cmd_choice
            if [ -n "$cmd_choice" ] && [ "$cmd_choice" -ge 1 ] && [ "$cmd_choice" -le "${#db_commands[@]}" ]; then
                selected_command="${db_commands[$((cmd_choice - 1))]%%|*}"
            fi
            ;;
        4)
            echo ""
            echo "Cache/Optimize Commands:"
            for i in "${!cache_commands[@]}"; do
                local num=$((i + 1))
                local cmd_info="${cache_commands[$i]}"
                local cmd_name="${cmd_info%%|*}"
                local cmd_desc="${cmd_info#*|}"
                echo "  $num) $cmd_name - $cmd_desc"
            done
            echo ""
            read -p "Select command [1-${#cache_commands[@]}]: " cmd_choice
            if [ -n "$cmd_choice" ] && [ "$cmd_choice" -ge 1 ] && [ "$cmd_choice" -le "${#cache_commands[@]}" ]; then
                selected_command="${cache_commands[$((cmd_choice - 1))]%%|*}"
            fi
            ;;
        5)
            echo ""
            read -p "Enter artisan command (e.g., 'tenants:list' or 'migrate --force'): " custom_cmd
            if [ -n "$custom_cmd" ]; then
                selected_command="$custom_cmd"
            fi
            ;;
        0)
            print_info "Operation cancelled"
            press_enter
            return
            ;;
        *)
            print_error "Invalid selection"
            press_enter
            return
            ;;
    esac
    
    if [ -z "$selected_command" ]; then
        print_error "No command selected"
        press_enter
        return
    fi
    
    # Ask for additional arguments if needed
    echo ""
    read -p "Enter additional arguments (e.g., '--tenants=1' or '--force') or press Enter for none: " command_args
    
    echo ""
    print_info "Running: php artisan $selected_command $command_args"
    echo ""
    echo "--------------------------------------------"
    
    # Run the command
    if [ -n "$command_args" ]; then
        docker exec "$container" php /var/www/html/artisan $selected_command $command_args
    else
        docker exec "$container" php /var/www/html/artisan $selected_command
    fi
    
    local exit_code=$?
    echo "--------------------------------------------"
    echo ""
    
    if [ $exit_code -eq 0 ]; then
        print_success "Command completed successfully"
    else
        print_error "Command failed with exit code $exit_code"
    fi
    
    press_enter
}

diagnose_tenant_seeding() {
    print_header "Diagnose Tenant Seeding Issues"
    local container="envelope_api"
    local worker_container="envelope_worker"
    
    echo "1. Checking Worker Status..."
    echo "--------------------------------------------"
    if docker ps --format "{{.Names}}" | grep -q "^${worker_container}$"; then
        print_success "Worker container is running"
        docker exec "$worker_container" supervisorctl status 2>&1 | grep queue || echo "Queue worker status not available"
    else
        print_error "Worker container is NOT running!"
        print_info "Start it with: docker compose up -d worker"
    fi
    echo ""
    
    echo "2. Checking Queue Configuration..."
    echo "--------------------------------------------"
    docker exec "$container" php /var/www/html/artisan tinker --execute="
        echo 'Queue Connection: ' . config('queue.default') . PHP_EOL;
        echo 'Queue Driver: ' . config('queue.connections.' . config('queue.default') . '.driver') . PHP_EOL;
    " 2>&1
    echo ""
    
    echo "3. Checking Pending Jobs..."
    echo "--------------------------------------------"
    docker exec "$container" php /var/www/html/artisan tinker --execute="
        try {
            \$pending = DB::table('jobs')->count();
            echo 'Pending jobs: ' . \$pending . PHP_EOL;
            if (\$pending > 0) {
                \$tenant_jobs = DB::table('jobs')
                    ->whereRaw('payload LIKE ?', ['%tenant%'])
                    ->count();
                echo 'Tenant-related jobs: ' . \$tenant_jobs . PHP_EOL;
            }
        } catch (Exception \$e) {
            echo 'Error: ' . \$e->getMessage() . PHP_EOL;
        }
    " 2>&1
    echo ""
    
    echo "4. Checking Failed Jobs..."
    echo "--------------------------------------------"
    docker exec "$container" php /var/www/html/artisan queue:failed 2>&1 | head -20
    echo ""
    
    echo "5. Checking Recent Laravel Logs for Errors..."
    echo "--------------------------------------------"
    docker exec "$container" tail -50 /var/www/html/storage/logs/laravel.log 2>/dev/null | grep -i -E "(error|exception|failed|tenant)" | tail -10 || echo "No recent errors found"
    echo ""
    
    echo "6. Checking Worker Logs..."
    echo "--------------------------------------------"
    if docker ps | grep -q "$worker_container"; then
        docker exec "$worker_container" tail -30 /var/www/html/storage/logs/laravel.log 2>/dev/null | grep -i -E "(queue|job|tenant)" | tail -10 || echo "No relevant logs found"
    else
        echo "Worker container not running"
    fi
    echo ""
    
    echo "7. Testing Queue Processing..."
    echo "--------------------------------------------"
    print_info "Attempting to process one job..."
    if docker ps --format "{{.Names}}" | grep -q "^${worker_container}$"; then
        docker exec "$worker_container" php /var/www/html/artisan queue:work --once --verbose 2>&1 | head -30
    else
        print_error "Worker container not running - cannot test"
    fi
    echo ""
    
    print_header "Diagnostic Complete"
    echo ""
    echo "Common fixes:"
    echo "  - If worker is not running: docker compose up -d worker"
    echo "  - If jobs are stuck: docker compose restart worker"
    echo "  - If jobs failed: Use option 22 to retry failed jobs"
    echo "  - To manually seed: Use option 23"
    
    press_enter
}

# =========================================
# MAIN MENU
# =========================================

show_main_menu() {
    clear
    print_header "Docker Manager - $ENV_MODE Environment"
    echo ""
    echo "Current compose file: $(get_compose_file)"
    echo ""
    echo -e "${CYAN}Environment:${NC}"
    echo "  [E] Change Environment (Current: $ENV_MODE)"
    echo ""
    echo -e "${CYAN}Build & Deploy:${NC}"
    echo "  [1] Build Images"
    echo "  [2] Rebuild Images (no cache)"
    echo "  [3] Pull Images (with retry)"
    echo "  [4] Start Services"
    echo "  [5] Stop Services"
    echo "  [6] Restart Services"
    echo "  [7] Down Services (stop & remove)"
    echo ""
    echo -e "${CYAN}Image Management:${NC}"
    echo "  [8] List Images"
    echo "  [9] Delete Image"
    echo "  [10] Delete All Envelope Images"
    echo ""
    echo -e "${CYAN}Container Management:${NC}"
    echo "  [11] List Containers"
    echo "  [12] Show Logs"
    echo "  [13] Show Status"
    echo "  [14] Show Health"
    echo ""
    echo -e "${CYAN}Debugging & Testing:${NC}"
    echo "  [15] Diagnose Service (api/hq/tenant/all)"
    echo "  [16] Test Services"
    echo "  [17] Fix Service Cache (api/hq/tenant/all)"
    echo "  [18] Inspect Container"
    echo "  [19] Execute into Container"
    echo ""
    echo -e "${CYAN}Tenant & Queue Management:${NC}"
    echo "  [20] Check Queue Status"
    echo "  [21] Check Tenant Seeding Status"
    echo "  [22] Retry Failed Jobs"
    echo "  [23] Manually Seed Tenant"
    echo "  [24] Diagnose Tenant Seeding Issues"
    echo "  [25] Migrate Tenant Database (tenants:migrate)"
    echo "  [26] Fresh Migrate Tenant Database (tenants:migrate-fresh)"
    echo "  [27] Fresh Migrate and Seed Tenant Database"
    echo "  [28] Run Artisan Command (Interactive)"
    echo ""
    echo -e "${CYAN}Database Management:${NC}"
    echo "  [29] Fresh Migrate Central Database"
    echo "  [30] Fresh Migrate and Seed Central Database"
    echo ""
    echo -e "${CYAN}Configuration:${NC}"
    echo "  [31] Configure Docker Daemon"
    echo "  [32] Docker Cleanup"
    echo ""
    echo "  [Q] Quit"
    echo ""
}

# =========================================
# MAIN LOOP
# =========================================

main() {
    # Check if docker is available
    if ! command -v docker &> /dev/null; then
        print_error "Docker is not installed or not in PATH"
        exit 1
    fi
    
    # Check if docker-compose file exists
    if [ ! -f "$DEV_COMPOSE" ]; then
        print_error "Development compose file not found: $DEV_COMPOSE"
        print_info "Current directory: $(pwd)"
        print_info "Looking for: $DEV_COMPOSE"
        exit 1
    fi
    
    while true; do
        show_main_menu
        read -p "Select option: " choice
        
        case $choice in
            [Ee]) select_environment ;;
            1) build_images ;;
            2) rebuild_images ;;
            3) pull_images_with_retry ;;
            4) start_services ;;
            5) stop_services ;;
            6) restart_services ;;
            7) down_services ;;
            8) list_images ;;
            9) delete_image ;;
            10) delete_all_images ;;
            11) list_containers ;;
            12) show_logs ;;
            13) show_status ;;
            14) show_health ;;
            15) diagnose_service ;;
            16) test_services ;;
            17) fix_service_cache ;;
            18) inspect_container ;;
            19) exec_into_container ;;
            20) check_queue_status ;;
            21) check_tenant_seeding ;;
            22) retry_failed_jobs ;;
            23) manually_seed_tenant ;;
            24) diagnose_tenant_seeding ;;
            25) migrate_tenant ;;
            26) fresh_migrate_tenant ;;
            27) fresh_migrate_and_seed_tenant ;;
            28) run_artisan_command ;;
            29) fresh_migrate_central ;;
            30) fresh_migrate_and_seed_central ;;
            31) configure_docker ;;
            32) cleanup_docker ;;
            [Qq]) 
                print_info "Goodbye!"
                exit 0
                ;;
            *)
                print_error "Invalid option"
                sleep 1
                ;;
        esac
    done
}

# Run main function
main
